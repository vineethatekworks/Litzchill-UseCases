...
export default async function updateMemeStatus(req: Request, params: Record<string, string>, updateMemeStatusQueryFn = updateMemeStatusQuery) {
  ...
  try {
      ...
      if (!meme_status) return ErrorResponse(HTTP_STATUS_CODE.BAD_REQUEST, MEME_ERROR_MESSAGES.MISSING_STATUS_VALUE);
      
      // Validate `meme_status`
      const validStatuses = [MEME_STATUS.APPROVED,MEME_STATUS.REJECTED];
      if (!validStatuses.includes(meme_status))  return ErrorResponse(HTTP_STATUS_CODE.BAD_REQUEST, COMMON_ERROR_MESSAGES.INVALID_DATA);

      // Update meme status
      const { data: updatedMemeStatus, error } = await updateMemeStatusQueryFn(meme_id, meme_status,user_id);
      if (error || !updatedMemeStatus)   return ErrorResponse(HTTP_STATUS_CODE.INTERNAL_SERVER_ERROR, MEME_ERROR_MESSAGES.FAILED_TO_UPDATE);
